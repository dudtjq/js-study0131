...
import {caseUp, caseDown, currectAnswer } from "./checkAnswer.js";
...
export default function gameStart(){
  ...
  const makeIcon = () => {
    ...
    for(let n = 1; n <= 100; n++){
      ...
      $farg.appendChild($icon);
      
    }

    $numbers.appendChild($farg);
  ...
  $numbers.onclick = e => {
    ...
    //console.log(`사용자가 클릭한 이이콘 번호 : ${e.target.dataset.iconNumber}`);

    // 사용자가 클릭한 아이콘의 숫자를 answer에 저장(대소비교를 위해 정수를 변환)
